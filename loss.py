...
class ReproLoss:
    ...
    def __init__(self,
        ...
        self.soft_clamp = soft_clamp    # 50
        self.soft_clamp_min = soft_clamp_min # 1
        self.type = type
        self.circle_schedule = circle_schedule

    def compute(self, repro_errs_b1N, iteration, valid):
        ...
        if not valid:
            if self.type == "dyntanh":
                schedule_weight = iteration / self.total_iterations

                if self.circle_schedule:
                    schedule_weight = 1 - np.sqrt(1 - schedule_weight ** 2)
                loss_weight = (1 - schedule_weight) * self.soft_clamp * 2 + self.soft_clamp
                return weighted_tanh(repro_errs_b1N, loss_weight)
            else:
                raise TypeError("loss type error")
