...
if __name__ == '__main__':
    ...
    parser.add_argument('--network', type=Path,default='output_final_3/visual/7scenes_chess_visual.pt', 
    ...
    heads_params = network.get_head_param()
    ...
    # Setup output files.
    test_log = open(test_log_file, 'a', 1)
    ...
    if opt.render_visualization:
        ...
        target_path = vutil.get_rendering_target_path(
            ...
            opt.network)
        ace_visualizer = Visualizer(target_path,
    ...
    with torch.no_grad():
        for image_B1HW, _, gt_pose_B44, _, intrinsics_B33, _, _, filenames in testset_loader:
            ...
            with autocast(enabled=True):
                ...
                # heads_params, scene_coordinates_B3HW = network(image_B1HW)
                # head_end_time = time.time()
                # head_used_time = head_end_time - head_start_time
    ...
    _logger.info(f"heads_params: {heads_params}\n")

    # Write to the test log file as well.
    test_log.write(f"===================================================\nTest complete\n"
                   f"Accuracy:\n\t10cm/5deg: {pct10_5:.1f}%\n\t5cm/5deg: {pct5:.1f}%\n\t2cm/2deg: {pct2:.1f}%\n\t1cm/1deg: {pct1:.1f}%\n"
                   f"Median Error: {median_rErr:.1f}deg, {median_tErr:.1f}cm\nAvg. processing time: {avg_time * 1000:4.1f}ms\n"
                   f"heads_params: {heads_params}\n\n\n\n\n")
