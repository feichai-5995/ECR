...
from util import get_pixel_grid, to_homogeneous
from loss import ReproLoss
from network import Regressor
from dataset import CamLocDataset

import vis_util as vutil
...
class Trainer:
    ...
    def __init__(self, options):
        ...
        # self.optimizer = optim.Adadelta(self.regressor.parameters(), lr=self.options.learning_rate_min)
        # self.optimizer = optim.Nadam(self.regressor.parameters(), lr=self.options.learning_rate_min)
        ...
        if self.options.render_visualization:
            ...
            self.ace_visualizer = Visualizer(
                target_path,
    ...
    def training_step(self, features_bC, target_px_b2, gt_inv_poses_b34, Ks_b33, invKs_b33):
        ...
        loss_valid = self.repro_loss.compute(valid_reprojection_error_b1, self.iteration, True)
        ...
        # loss_invalid_1 = torch.sqrt(torch.sum((target_camera_coords_b31 - pred_cam_coords_b31)**2, dim=1)).masked_select(invalid_mask_b1).sum()

        # Final loss is the sum of all 2.
        # loss = loss_valid + loss_invalid + loss_invalid_1
        ...
        invalid_min_depth_b1 = pred_cam_coords_b31[:, 2] < self.options.depth_min
        invalid_max_depth_b1 = pred_cam_coords_b31[:, 2] > self.options.depth_max
        invalid_mask_b1_1 = (invalid_min_depth_b1 | invalid_max_depth_b1).squeeze()
        valid_mask_b1_1 = ~invalid_mask_b1_1
        pred_px_b31 = pred_px_b31[valid_mask_b1_1]
        
        pred_px_b21 = pred_px_b31[:, :2] / pred_px_b31[:, 2, None]
        reprojection_error_b2 = pred_px_b21.squeeze() - target_px_b2[valid_mask_b1_1]
        reprojection_error_b1 = torch.norm(reprojection_error_b2, dim=1, keepdim=True, p=1)
        invalid_mask_b1_2 = (reprojection_error_b1 > self.options.repro_loss_hard_clamp).squeeze()
        valid_mask_b1_2 = ~invalid_mask_b1_2
        valid_reprojection_error_b1 = reprojection_error_b1[valid_mask_b1_2]
        
        pixel_grid_crop_b31 = to_homogeneous(target_px_b2.unsqueeze(2))
        target_camera_coords_b31 = self.options.depth_target * torch.bmm(invKs_b33, pixel_grid_crop_b31)

        target_camera_coords_b31_1 = target_camera_coords_b31[invalid_mask_b1_1]
        target_camera_coords_b31_2 = (target_camera_coords_b31[valid_mask_b1_1])[invalid_mask_b1_2]
        pred_cam_coords_b31_1 = pred_cam_coords_b31[invalid_mask_b1_1]
        pred_cam_coords_b31_2 = (pred_cam_coords_b31[valid_mask_b1_1])[invalid_mask_b1_2]
        target_camera_coords_b31_invalid = torch.cat((target_camera_coords_b31_1, target_camera_coords_b31_2),dim=0)
        pred_cam_coords_b31_invalid = torch.cat((pred_cam_coords_b31_1, pred_cam_coords_b31_2),dim=0)
        
        loss_valid = self.repro_loss.compute(valid_reprojection_error_b1, self.iteration, True)
        loss_invalid_1 = torch.abs(target_camera_coords_b31_invalid - pred_cam_coords_b31_invalid).sum()
        loss_invalid_2 = torch.sqrt(torch.sum((target_camera_coords_b31_invalid - pred_cam_coords_b31_invalid)**2, dim=1)).sum()
        # loss_invalid_1 = torch.sum(torch.abs(target_camera_coords_b31_invalid - pred_cam_coords_b31_invalid),dim=1)
        # loss_invalid_2 = torch.sqrt(torch.sum((target_camera_coords_b31_invalid - pred_cam_coords_b31_invalid)**2, dim=1))
        # loss_invalid = loss_invalid_1 + loss_invalid_2
        # if loss_invalid.mean() < self.options.repro_loss_hard_clamp * 5:
        #     loss_invalid = self.repro_loss.compute(loss_invalid, self.iteration, False)
        # loss_invalid = self.repro_loss.compute(loss_invalid, self.iteration, False)

        valid_mask_b1 = batch_size - (invalid_mask_b1_1.sum() + invalid_mask_b1_2.sum())

        # loss = loss_valid + loss_invalid
        loss = loss_valid + (loss_invalid_1 + loss_invalid_2) / 2
        # loss = loss_valid + loss_invalid_1
        loss /= batch_size
