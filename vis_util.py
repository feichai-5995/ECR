...
def get_point_cloud_from_network(network, data_loader, filter_depth):
    ...
    with torch.no_grad():
        ...
        for image, _, _, gt_inv_pose, K, _, _, file in data_loader:
            ...
            gt_inv_pose = gt_inv_pose.to(torch.device('cuda:1'))
            K = K.to(torch.device('cuda:1'))
            ...
            reprojection_error_2N = pred_px_B2N.squeeze() - pixel_positions_2N.to(torch.device('cuda:1'))
